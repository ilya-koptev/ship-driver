...
| Контрол (топик) | Свет | pwm8a04 | Канал | Выход | Разъём (пин) | Ключи ULN |
|---|---|---|---|---|---|---|
| `nav_lights`   | Navigation lights (ходовые огни)   | a11 | ch3 | OUT1 | X5 (3) | O1 |
| `morse_lamp`   | Morse signal lamp (прожектор морзе) | a12 | ch3 | OUT2 | X5 (2) | O2 |
| `deck_lights`  | Deck lights (палубные огни)         | a13 | ch1 | OUT3 | X5 (1) | O3 |
| `cabin_light1` | Cabin light 1 (внутренний свет 1)   | a13 | ch2 | OUT4 | X8 (1) | **O4 ‖ O5** |
| `cabin_light2` | Cabin light 2 (внутренний свет 2)   | a13 | ch3 | OUT5 | X8 (2) | **O6 ‖ O7** |

> ⚡ **OUT4 и OUT5 — сдвоенные:** на каждый параллельно работают **два** ключа ULN (O4+O5 и
> O6+O7), поэтому они держат примерно **вдвое больший ток** — ставь на них более мощные
> нагрузки.

**Выходные разъёмы:** `X5` (4 пина) — `OUT1`/`OUT2`/`OUT3` + **+12 В**; `X8` (3 пина) —
`OUT4`/`OUT5` + **+12 В**.

**Токи (ULN2003A):**

- `OUT1`–`OUT3` (одиночный ключ) — до **~500 мА** длительно (пик 600 мА).
- `OUT4`, `OUT5` (два ключа параллельно) — до **~1 А**.
...
**Как пользоваться:** «+» нагрузки — на **+12 В** платы (на разъёме X5/X8), «−» — на нужный
выход `OUTx`. Яркость задаётся ШИМ 0–100 % через контрол света (`nav_lights` … `cabin_light2`).
Мощную нагрузку бери на сдвоенные `OUT4`/`OUT5`; если нужно ещё больше — внешний MOSFET.
