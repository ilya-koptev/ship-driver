...
| `nav_lights` / `morse_lamp` / `deck_lights` / `cabin_light1` / `cabin_light2` | зап. | свет, 0–100 % (разводка — см. [«Подключение света»](#подключение-света-uln2003)) |
...
## Подключение света (ULN2003)

Пять световых каналов коммутирует одна сборка Дарлингтона **ULN2003A** (7 ключей с открытым
коллектором, ток «втекающий» — ключ замыкает выход на землю). ШИМ 5 В с каналов `pwm8a04`
заходит на входы `INx`, нагрузка включается между «плюсом» питания и выходом `OUTx`. Общий
вывод `COM` (pin 9) — катоды встроенных защитных диодов — заводится на «плюс» питания нагрузки.

| Контрол (топик) | Название | pwm8a04 | Канал | Выход ULN2003 |
|---|---|---|---|---|
| `nav_lights` | Navigation lights (ходовые огни) | a11 | ch3 | OUT1 |
| `morse_lamp` | Morse signal lamp (прожектор морзе) | a12 | ch3 | OUT2 |
| `deck_lights` | Deck lights (палубные огни) | a13 | ch1 | OUT3 |
| `cabin_light1` | Cabin light 1 (внутренний свет 1) | a13 | ch2 | OUT4 |
| `cabin_light2` | Cabin light 2 (внутренний свет 2) | a13 | ch3 | OUT5 |

**Токи (даташит ULN2003A):**

- **На канал** — до **500 мА** длительно (пик 600 мА), допустимое напряжение коллектора до **50 В**.
- Падение на открытом ключе `VCE(sat)` ≈ 1.1 В @ 200 мА … 1.6 В @ 350 мА; на ключе рассеивается `P = VCE(sat)·I` (греется).
- **Суммарно по корпусу** мощность ограничена (≈ 1 Вт при 25 °C), поэтому держать одновременно все каналы по 500 мА нельзя — действует деретинг «допустимый ток на канал ↓ при росте числа одновременно открытых выходов». Ориентир: суммарно `Σ(VCE(sat)·I) ≲ 1 Вт`, т.е. при всех 5 включённых постоянно — **≈ до 150 мА на канал**. ШИМ со скважностью < 100 % снижает средний нагрев.
- Входы рассчитаны на логику (внутренний резистор 2.7 кОм в базе) — совместимы с 5 В ШИМ `pwm8a04`.

Для мощной/яркой нагрузки сверх деретинга — внешний MOSFET; ULN2003A оптимален для маломощных огней и сигнальных ламп.
