...
`/etc/ship-driver-state.json` и переживает перезагрузку. **При старте драйвер записывает
LoRa‑план из конфига (группа «LoRa») в каждый MOD‑модем** (и читает для подтверждения) — то
есть конфиг главный: что в настройках, то и в модуле. Адрес затем можно менять на лету через
`ship_number` (на следующем старте вернётся к плановому).
