...
**читаются** из самих модемов — драйвер их не перезаписывает (запись в модем сбивает
установленную радиосвязь). Канал/скорость модема меняются по проводу через Ship Setup или
прошивкой модуля; адрес — на лету через `ship_number`.
...
| Контрол | Свет | pwm8a04 | Канал | Выход ULN2003 |
|---|---|---|---|---|
| `light1` | Ходовые огни (Navigation lights) | a11 | ch3 | OUT1 |
| `light2` | Прожектор морзе (Morse signal lamp) | a12 | ch3 | OUT2 |
| `light3` | Палубные огни (Deck lights) | a13 | ch1 | OUT3 |
| `light4` | Внутренний свет 1 (Cabin light 1) | a13 | ch2 | OUT4 |
| `light5` | Внутренний свет 2 (Cabin light 2) | a13 | ch3 | OUT5 |
...
задать «Номер корабля» → **«Записать»** (или «Считать» для проверки) → вернуть модем в **RUN**.
Канал MODn с тем же каналом/скоростью подхватит корабль.
