...
Формат: `[slave] [func] [addr_hi] [addr_lo] [n_hi] [n_lo] [CRC_lo] [CRC_hi]`  
CRC — CRC16/Modbus (poly 0xA001, little-endian).

### battery_current (FC04)

```
TX → 0A 04 00 05 00 01 20 B0
     slave=10(UPS)  FC04  addr=UPS_CUR=5  n=1  CRC

RX ← 0A 04 02 FF 6E DC ED
     slave=10  FC04  byte_cnt=2  data=0xFF6E  CRC

decode: s16(0xFF6E) = −146 → × 0.001 = −0.146 A
→ MQTT: /devices/boat4/controls/battery_current = "-0.146"
```

### battery_temperature (FC04)

```
TX → 0A 04 00 09 00 01 E0 B3   (addr=UPS_TEMP=9)
decode: s16(raw) × 0.01 = °C
```

### charge_level (FC04)

```
TX → 0A 04 00 08 00 01 B1 73   (addr=UPS_CHG=8)
decode: raw × 0.01 = %
```

### pwm_alive — зондирование (FC03)

```
TX → 0B 03 00 00 00 01 84 A0
     slave=11(pwm8a04)  FC03  addr=FREQ_REG[1]=0  n=1  CRC
```

Опрашивает slave 11, 12, 13. Первый ответ = корабль онлайн (dаже без UPS).

### запись мотора front_right=60% (FC06)

```
TX → 0B 06 00 71 00 3C D9 6A
     slave=11  FC06  addr=DUTY_REG[2]=113(0x71)  val=60(0x3C)  CRC

RX ← 0B 06 00 71 00 3C D9 6A   (эхо = запрос)
← MQTT: /devices/boat4/controls/front_right/on = "60"
```

### DFPlayer — трек 3 (raw frame, не Modbus)

```
TX → 7E FF 06 08 00 00 03 EF
     START  VER  LEN  CMD=play  FB  param_hi  track=3  END
← MQTT: /devices/boat4/controls/mp3_track/on = "3"
```

---

## Регистры WB-UPS (slave 10)

| Константа | Адрес | FC | Назначение | Масштаб |
|---|---|---|---|---|
| `UPS_CUR` | 5 | FC04 | Ток батареи | × 0.001 A, s16 |
| `UPS_CHG` | 8 | FC04 | Заряд | × 0.01 % |
| `UPS_TEMP` | 9 | FC04 | Температура АКБ | × 0.01 °C, s16 |
| `UPS_CHG_SETPOINT` | 18 | FC06 | Ток заряда (мА) | 2000 / 600 |

## Регистры pwm8a04 (slave 11 / 12 / 13)

| Константа | CH1 | CH2 | CH3 | Назначение |
|---|---|---|---|---|
