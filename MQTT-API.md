...
Внешние клиенты (управление кораблём) подключаются к брокеру `192.168.69.105:1883`. На этом порту доступны **только топики корабля** — `/devices/boatN/#`, `/devices/chargerN/#`, `/devices/ship_setup|ship_diag|ship_bus/#` (чтение и запись). Прочие топики контроллера (`/devices/system`, `/devices/network`, `wb-*` и т. п.) на этом порту закрыты.

Сам API топиков и контролов **не менялся** — управление и телеметрия те же, что и раньше; отличается лишь область видимости на внешнем порту.
